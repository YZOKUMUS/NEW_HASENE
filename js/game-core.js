...
window.addEventListener('DOMContentLoaded', () => {
    ...
    <!-- Özel Onay Pop-up -->
<script>
// Arapça hareke renklendirme fonksiyonu (boş - kullanılmıyor ama çağrılıyor)
...
<div class="nav-item active" onclick="showMainMenu(); setActiveNavItem(0);">
    <div class="nav-icon">🏠</div>
    <div class="nav-label">Ana Sayfa</div>
</div>
<div class="nav-item" onclick="showStatsModal(); setActiveNavItem(1);">
    <div class="nav-icon">📊</div>
    <div class="nav-label">İstatistikler</div>
</div>
<div class="nav-item" onclick="showBadgesModal(); setActiveNavItem(3);">
    <div class="nav-icon">🏆</div>
    <div class="nav-label">Başarılar</div>
</div>
<div class="nav-item" onclick="showDailyTasksModal(); setActiveNavItem(2);" id="navTasks">
    <div class="nav-icon">✅</div>
    <div class="nav-label">Görevler</div>
    <span class="nav-badge" id="tasksBadge" style="display: none;">!</span>
</div>
<div class="nav-item" onclick="showCalendarModal(); setActiveNavItem(4);">
    <div class="nav-icon">📅</div>
    <div class="nav-label">Takvim</div>
</div>
    </div>
...
window.__testKelimeFlow = async function() {
        ...
        if (!opts || opts.length === 0) {
            log.warn('⚠️ No options rendered yet; ensure kelime data is loaded');
            return false;
        }
        // Click first option
        opts[0].click();
...
// GitHub Pages için dinamik base path
const basePath = window.location.pathname.includes("NEW_HASENE") ? "/NEW_HASENE/" : "./";
const swPath = basePath === "./" ? "./sw.js" : "/NEW_HASENE/sw.js";

// Önce mevcut Service Worker'ları kontrol et ve güncelle
navigator.serviceWorker.getRegistrations().then((registrations) => {
    // Tüm eski Service Worker'ları kaldır (yeniden kayıt için)
    return Promise.all(
        registrations.map((registration) => {
            log.debug("🔄 Eski Service Worker kaldırılıyor...");
    ...
    return navigator.serviceWorker.register(swPath, { 
        scope: basePath,
        updateViaCache: 'none' // Her zaman yeni versiyonu kontrol et
    });
}).then((registration) => {
    log.debug("✅ Yeni Service Worker kaydedildi ✔", swPath);
    ...
    registration.addEventListener('updatefound', () => {
        ...
        newWorker.addEventListener('statechange', () => {
            if (newWorker.state === 'activated') {
                log.debug("🔄 Yeni Service Worker aktif! Sayfa yenileniyor...");
                // Yeni Service Worker aktif olduğunda sayfayı yenile
    ...
    log.error("❌ Service Worker kayıt hatası:", err);
});
    }
